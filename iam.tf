...
resource "aws_iam_role_policy" "cross_account" {
  ...
  policy = jsonencode({
    ...
    "Statement" : [
      ...
      },
      {
        ...
        "Resource" : "arn:aws:ec2:${var.region}:${local.aws_account_id}:security-group/${aws_security_group.databricks_classic_compute.id}",
        "Condition" : {
          "StringEquals" : {
            "ec2:vpc" : "arn:aws:ec2:${var.region}:${local.aws_account_id}:vpc/${var.vpc_id}"
          }
  ...
  depends_on = [
    aws_security_group.databricks_classic_compute
  ]
}